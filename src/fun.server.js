    ...
    constructor({
        ...
        this.WebSocket.on('connection', (socket) => {
            ...
            socket.on('message', async (data) => {
                ...
                if (data.type == 'verify-password') {
                    if (this.password) return reply({
                        status: true, require: true
                    }); else return reply({
                        status: true, require: false
                    })
                }

                if (!socket.verify && this.password) {
                    if (data.type !== 'password') {
                        if (socket['@retryCount!type'])
                            socket['@retryCount!type'] = 0
                        socket['@retryCount!type'] += 1
                        if (socket['@retryCount!type'] >= 10)
                    ...
                    if (data?.payload[0] !== this.password) {
                        if (socket['@retryCount.type'])
                            socket['@retryCount.type'] = 0
                        socket['@retryCount.type'] += 1
                        if (socket['@retryCount.type'] >= 10)
